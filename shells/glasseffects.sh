#!/bin/bash
#
# Developed by Fred Weinhaus 6/24/2009 .......... revised 6/24/2009
#
...
trap "rm -f $tmpA1 $tmpB1 $tmpA2 $tmpB2 $tmp1 $tmp2 $tmp3; exit 0" 0
...
if [ $amount -gt 0 ]; then
	...
	if [ "$effect" = "displace" ]; then
		composite $tmp1 $tmpA1 \
			-displace ${amount}x${amount} \
			$tmpA1
		...
		if [ "$im_version" -lt "06040808" ]; then
			...
			convert $tmpA1 \
				...
				-compose displace -set option:compose:args ${amount}x${amount} -composite \
				$tmpA1
		fi
	fi
fi
...
if [ $weight -gt 0 ]; then
	...
	if [ "$kind" = "simple" ]; then
		...
		convert -size ${xcell}x${ycell} pattern:checkerboard -scale ${ww}x${hh}! \
			-contrast-stretch 0 -edge ${weight} \
			$tmpA2
