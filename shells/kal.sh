#!/bin/bash
#
# Revised by Fred Weinhaus ................... 7/1/2011
...
# [-B blend] [-n newseed] [infile] outfile
...
if [ "$infile" = "" ]; then
	...
	# test input image
	convert -quiet -regard-warnings "$infile" $cropping +repage "$tmpA1" ||
		errMsg "--- FILE $infile DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE ---"
elif [ "$mode" = "random" ]; then
	# test input image
	convert -quiet -regard-warnings "$infile" "$tmpA1" ||
		errMsg "--- FILE $infile DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE ---"
	convert $tmpA1 $cropping +repage \( +clone $seeding +noise random \) \
	...
	convert -quiet -regard-warnings "$infile" $cropping +repage "$tmpA1" ||
		errMsg "--- FILE $infile DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE ---"
	convert $tmpA1 $seed +noise Random -virtual-pixel tile $smooth \
